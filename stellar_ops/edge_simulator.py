...
def main() -> None:
    ...
    with socket.create_connection((args.host,args.port),timeout=3) as sock:
        file=sock.makefile("rwb"); file.write(encode_frame(hello(args.device,boot,"sim-1.0",["chamber_pressure","thrust","case_temperature","continuity"]))); file.flush(); print(decode_frame(file.readline()))
        for sequence in range(args.batches):
            ...
            msg=batch(args.device,boot,sequence,int(base*1_000_000),period_us,{"chamber_pressure":pressure,"thrust":thrust,"case_temperature":temperature,"continuity":[0]*size})
