(() => {
  ...
  const readRadius = () => { const r=Number(localStorage.getItem(RADIUS_KEY)||50); return RADII.includes(r)?r:50; };
  ...
  function panelBody(){
    ...
    const rows=ac.slice(0,60).map(a=>{ const sev=severity(a.distance_km); return `<tr data-air12-hex="${esc(a.hex)}" class="${state.selected===a.hex?'selected':''}"><td><code>${esc(a.callsign||'—')}</code><small>${esc(a.aircraft_type||a.registration||'—')}</small></td><td class="air12-${sev.toLowerCase()}">${fmt(a.distance_km,1,' km')}<small>${fmt(a.bearing_deg,0,'°')}</small></td><td>${fmt(a.altitude_ft,0,' ft')}<small>${fmt(a.ground_speed_kmh,0,' km/h')}</small></td><td>${fmt(a.track_deg,0,'°')}<small>${trend(a)}</small></td></tr>`; }).join('');
  ...
  async function loadTraffic(){
    ...
    try{
      ...
      if(e.name==='AbortError'){
        if(!state.lastUpdated){ state.status='TIMEOUT'; state.message='Traffic source timed out. Retry or wait for the next automatic update.'; }
      }else{
  ...
  function drawMap(){
    ...
    for(let tx=Math.floor(left/256)-1;tx<=Math.floor((left+w)/256)+1;tx++) for(let ty=Math.floor(top/256)-1;ty<=Math.floor((top+h)/256)+1;ty++){
      if(ty<0||ty>=n) continue;
      const x=((tx%n)+n)%n,img=document.createElement('img');
      img.className='tile'; img.alt=''; img.draggable=false; img.src=`/api/airspace/tile/${z}/${x}/${ty}.png`; img.style.left=`${tx*256-left}px`; img.style.top=`${ty*256-top}px`; map.appendChild(img);
    }
  ...
  document.addEventListener('click',e=>{
    ...
    if(t.id==='air12-save'){
      ...
      state.cfg={site_name:name||'Operation Site',latitude:lat,longitude:lon}; persist(state.cfg); state.traffic=[]; state.lastUpdated=null; state.status='IDLE'; state.message='Waiting for first traffic update…'; renderPanelOnly(); queueMap(true); setTimeout(loadTraffic,0); return;
    }
    if(t.id==='air12-refresh'){ e.preventDefault(); loadTraffic(); return; }
    if(t.id==='air12-change'){ e.preventDefault(); if(state.request)state.request.abort(); state.traffic=[];state.lastUpdated=null;state.selected=null;state.status='IDLE';state.message='Waiting for first traffic update…';state.cfg=null;try{localStorage.removeItem(STORAGE_KEY);}catch(_){} if(state.map)state.map.style.display='none';renderPanelOnly();return; }
    if(t.dataset?.air12Hex){ state.selected=t.dataset.air12Hex; renderPanelOnly(); queueMap(true); return; }
    if(t.classList?.contains('plane')&&t.dataset.hex){ state.selected=t.dataset.hex; renderPanelOnly(); queueMap(true); }
  },{capture:true,signal});
  ...
  document.addEventListener('change',e=>{
    ...
    renderPanelOnly();queueMap(true);setTimeout(loadTraffic,0);
  },{signal});
  ...
  if(workspace){
    state.observer=new MutationObserver(()=>queueMap());
