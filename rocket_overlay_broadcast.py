...
BROADCAST_THEMES = {
    "launch": "Launch Broadcast",
    "mission_control": "Mission Control",
    "stellar_console": "Stellar Kinetics Broadcast Console",
}

THEME_ALIASES = {
    # Drafts saved before the original template-3 file was supplied used this
    # identifier for the third card. Keep them working, but render the correct
    # template D rather than the mistakenly inferred chart layout.
    "range_line": "stellar_console",
}


def normalize_broadcast_theme(theme: str) -> str:
    return THEME_ALIASES.get(theme, theme)


class BroadcastOverlayError(RuntimeError):
    """Recoverable configuration/rendering error for web integration."""
...
def fail(message: str, code: int = 2) -> None:
    raise BroadcastOverlayError(message)
...
def build_assets(cfg: Config, telemetry: pd.DataFrame) -> OverlayAssets:
    ...
    s = H / 1080.0
    theme = normalize_broadcast_theme(getattr(cfg, "broadcast_theme", "launch"))
    if theme not in BROADCAST_THEMES:
        fail(f"Unknown broadcast theme: {theme}")

    # Gradient scrims as a brightness multiplier column (black scrims).
    mult = np.ones((H, 1, 1), dtype=np.float32)
    if theme == "mission_control":
        top_h = int(240 * s)
        ...
        for i in range(bot_h):
            ...
            a = (
                0.35 * min(1.0, r / 0.4)
                if r < 0.4
                else 0.35 + 0.47 * ((r - 0.4) / 0.6)
            )
            mult[y] *= 1 - a
    elif theme == "stellar_console":
        top_h = int(190 * s)
        for y in range(top_h):
            mult[y] *= 1 - 0.72 * (1 - y / top_h)
        bot_h = int(330 * s)
    ...
    # Each theme shares the same trace data but places it differently.
    if theme == "mission_control":
        ...
        chart_y = H - int(44 * s) - int(58 * s) - chart_h
    ...
    pressure_span = max(p_peak * 1.05, 1e-9)
    ys = (chart_h - 2) - (np.clip(cp, 0, None) / pressure_span) * (chart_h - 12)
    ...
    return OverlayAssets(
        ...
        has_thrust=bool(telemetry.attrs.get("has_thrust", True)),
    )
...
def config_text(cfg: Config, name: str, default: str = "") -> str:
    """Read optional web metadata while preserving standalone-template defaults."""
    value = getattr(cfg, name, default)
    text = "" if value is None else str(value).strip()
    return text or default


def test_metadata(cfg: Config) -> str:
    """Compact user-supplied test identity used beneath the mission title."""
    return "  |  ".join(
        value
        for value in (
            config_text(cfg, "run_number"),
            config_text(cfg, "motor_type"),
            config_text(cfg, "test_date"),
        )
        if value
    )
...
def thrust_display(
    assets: OverlayAssets, thrust: float, unit: str, decimals: int = 0
) -> tuple[str, str]:
    """Never present a missing thrust channel as a measured zero."""
    if not assets.has_thrust:
        return "N/A", ""
    return f"{thrust:.{decimals}f}", unit
...
def compose_frame(
    ...
    metadata = test_metadata(cfg)
    if metadata:
        draw_text(
            frame, metadata, (x, sc(A, 148)), 0.42 * A.s,
            (150, 145, 140), 1, FONT_MONO,
        )
    ...
    thrust_value, thrust_unit = thrust_display(A, thrust, cfg.thrust_unit)
    draw_gauge(
        frame, A, g2_cx, g_cy,
        thrust / (A.f_peak * 1.05) if A.f_peak else 0,
        thrust_value, thrust_unit, "THRUST", WHITE,
    )
    ...
    if cfg.show_chart:
        ...
        peak_thrust = (
            f"{A.f_peak:.1f} {cfg.thrust_unit}" if A.has_thrust else "N/A"
        )
        draw_text(frame, peak_thrust, (px2, py + sc(A, 26)), 0.6 * A.s, WHITE, 1)

    return frame


# Keep the established layout as a named compositor, then expose a single
# dispatcher below.  Both the SDR renderer and the HDR alpha-matte path call
# the dispatcher, so the selected theme cannot drift between preview/export.
compose_frame_launch = compose_frame
...
def draw_bar_meter(
    ...
    """Mission Control horizontal telemetry meter."""
    A = assets
    draw_spaced_text(
        img, label, (x, y), 0.42 * A.s, (200, 200, 200), 1, sc(A, 3)
    )
    unit_width, _ = text_size(unit, 0.42 * A.s, 1)
    draw_text(img, unit, (x + width - unit_width, y), 0.42 * A.s, GRAY, 1)
    draw_text(img, value_text, (x - sc(A, 4), y + sc(A, 62)), 1.8 * A.s, WHITE, 3)
    bar_y = y + sc(A, 84)
    bar_height = max(2, sc(A, 6))
    cv2.rectangle(
        img, (x, bar_y), (x + width, bar_y + bar_height), (70, 62, 55), -1
    )
    fill_width = int(width * float(np.clip(fraction, 0, 1)))
    if fill_width > 0:
        cv2.rectangle(
            img, (x, bar_y), (x + fill_width, bar_y + bar_height), color, -1
        )
    limit_x = x + int(width * 0.95)
    cv2.line(
        img,
        (limit_x, bar_y - sc(A, 4)),
        (limit_x, bar_y + bar_height + sc(A, 4)),
        (160, 160, 160),
        max(1, sc(A, 2)),
        cv2.LINE_AA,
    )
    draw_text(
        img, "0", (x, bar_y + sc(A, 26)),
        0.38 * A.s, (115, 108, 100), 1,
    )
    peak_width, _ = text_size(peak_text, 0.38 * A.s, 1)
    draw_text(
        img,
        peak_text,
        (x + width - peak_width, bar_y + sc(A, 26)),
        0.38 * A.s,
        (115, 108, 100),
        1,
    )
...
def draw_strip_chart(
    ...
    """Mission Control full-width live pressure trace."""
    A = assets
    x, y, width, height = A.chart_x, A.chart_y, A.chart_w, A.chart_h
    roi = frame[y:y + height, x:x + width]
    faint = roi.copy()
    cv2.line(faint, (0, height - 1), (width, height - 1), WHITE, 1, cv2.LINE_AA)
    cv2.line(faint, (0, height // 2), (width, height // 2), WHITE, 1, cv2.LINE_AA)
    cv2.polylines(
        faint, [A.chart_pts], False, WHITE, max(1, sc(A, 2)), cv2.LINE_AA
    )
    frame[y:y + height, x:x + width] = cv2.addWeighted(
        faint, 0.30, roi, 0.70, 0
    )

    cursor_x = int(np.clip(t / A.t_chart_max, 0, 1) * (width - 1))
    revealed = A.chart_pts[A.chart_pts[:, 0] <= cursor_x]
    if revealed.shape[0] >= 2:
        cv2.polylines(
    ...
    cursor_y = (
        int(np.interp(cursor_x, A.chart_pts[:, 0], A.chart_pts[:, 1]))
        if A.chart_pts.shape[0]
        else height
    )
    cv2.circle(
        frame, (x + cursor_x, y + cursor_y), sc(A, 5), A.accent, -1, cv2.LINE_AA
    )
    cv2.circle(
        frame, (x + cursor_x, y + cursor_y), sc(A, 5), WHITE, 1, cv2.LINE_AA
    )
    draw_spaced_text(
        frame,
        f"CHAMBER PRESSURE - {cfg.pressure_unit}",
        (x, y - sc(A, 14)),
        0.42 * A.s,
        (200, 200, 200),
        1,
    ...
    duration_label = f"T 0-{A.t_chart_max:.0f} s"
    label_width, _ = text_size(duration_label, 0.42 * A.s, 1)
    draw_text(
        frame,
        duration_label,
        (x + width - label_width, y - sc(A, 14)),
        0.42 * A.s,
        GRAY,
        1,
    )
...
def compose_frame_mission_control(
    video_frame: np.ndarray,
    cfg: Config,
    assets: OverlayAssets,
    t: float,
    ...
    """Mission-control layout copied faithfully from broadcast variant 1."""
    width, height = cfg.width, cfg.height
    A = assets
    frame = cover_resize(video_frame, width, height).astype(np.float32)
    frame *= A.scrim_mult
    frame = frame.astype(np.uint8)
    margin = sc(A, 64)

    meter_width = sc(A, 330)
    meter_y = height // 2 - sc(A, 190)
    phase_width = sc(A, 300)
    phase_x = width - margin - phase_width
    phase_y = height // 2 - sc(A, 170)

    # Mission clock and state form one centered lockup.
    clock = format_clock(t)
    clock_scale = 1.9 * A.s
    clock_width, _ = text_size(clock, clock_scale, 3)
    center_x = width // 2
    draw_text(
        frame,
        clock,
        (center_x - clock_width // 2, sc(A, 96)),
        clock_scale,
        WHITE,
        3,
    )
    mission_width = draw_spaced_text(
        frame, cfg.title, (-10000, -10000), 0.55 * A.s, WHITE, 1, sc(A, 6)
    )
    mission_x = center_x - mission_width // 2
    mission_y = sc(A, 138)
    draw_spaced_text(
        frame,
        cfg.title,
    ...
    cv2.line(
        frame,
        (mission_x - sc(A, 74), mission_y - sc(A, 6)),
        (mission_x - sc(A, 18), mission_y - sc(A, 6)),
        (110, 105, 100),
        1,
        cv2.LINE_AA,
    )
    cv2.line(
        frame,
        (mission_x + mission_width + sc(A, 18), mission_y - sc(A, 6)),
        (mission_x + mission_width + sc(A, 74), mission_y - sc(A, 6)),
        (110, 105, 100),
        1,
        cv2.LINE_AA,
    )
    ...
    if t < 0:
        status, status_color = "PRE-IGNITION", GRAY
    elif t <= A.t_end + 0.1:
        status, status_color = "TEST ACTIVE", GREEN
    else:
        status, status_color = "TEST COMPLETE", YELLOW
    status_width = draw_spaced_text(
        frame, status, (-10000, -10000), 0.5 * A.s, status_color, 1, sc(A, 4)
    )
    ...
    cv2.circle(
        frame,
        (status_x - sc(A, 18), status_y - sc(A, 6)),
        sc(A, 4),
        tuple(int(channel * pulse) for channel in status_color),
        -1,
        cv2.LINE_AA,
    )

    # Compact identity in the opposite corner balances the centered clock.
    identity_x = margin
    if A.logo is not None:
        alpha_blend(frame, A.logo, identity_x, sc(A, 52), 0.95)
        identity_x += A.logo.shape[1] + sc(A, 20)
    else:
        box = sc(A, 52)
        cv2.rectangle(
            frame,
            (identity_x, sc(A, 56)),
            (identity_x + box, sc(A, 56) + box),
            (200, 200, 200),
            1,
            cv2.LINE_AA,
        )
        logo_width, logo_height = text_size("LOGO", 0.35 * A.s, 1)
        draw_text(
            frame,
            "LOGO",
            (
                identity_x + (box - logo_width) // 2,
                sc(A, 56) + (box + logo_height) // 2,
            ),
            0.35 * A.s,
            GRAY,
            1,
        )
        identity_x += box + sc(A, 20)
    draw_spaced_text(
        frame,
        cfg.subtitle.upper(),
        (identity_x, sc(A, 88)),
    ...
    if metadata:
        draw_text(
            frame, metadata, (identity_x, sc(A, 116)), 0.4 * A.s,
            (150, 145, 140), 1, FONT_MONO,
        )
    ...
    draw_bar_meter(
        ...
        thrust_value,
        thrust / (A.f_peak * 1.05) if A.f_peak else 0,
        f"PEAK {A.f_peak:.1f}" if A.has_thrust else "PEAK N/A",
        WHITE,
    )
    ...
    if cfg.show_phases:
        phases = [
            ("T-COUNT", None),
            ("IGNITION", 0.0),
            ("BURN", min(0.6, A.t_end * 0.1)),
            ("BURNOUT", A.t_end),
        ]
        active = max(
            index
            for index, phase in enumerate(phases)
            if phase[1] is None or t >= phase[1]
        )
        row_height = sc(A, 48)
        for index, (label, phase_time) in enumerate(phases):
            row_y = phase_y + index * row_height
            dot_color = A.accent if index <= active else (48, 44, 40)
            cv2.circle(
                frame,
                (phase_x + sc(A, 5), row_y - sc(A, 6)),
                sc(A, 5),
                dot_color,
                -1,
                cv2.LINE_AA,
            ...
            text_color = (
                WHITE
                if index == active
                else ((155, 150, 145) if index < active else (95, 90, 85))
            )
            draw_spaced_text(
                frame,
                label,
                (phase_x + sc(A, 26), row_y),
                0.46 * A.s,
                text_color,
                1,
                sc(A, 3),
            ...
            phase_text = (
                ...
                if phase_time is None
                    ...
                    f"T- {abs(phase_time):.1f}s"
                    if phase_time < 0
                    else f"T+ {phase_time:.1f}s"
                )
            )
            phase_text_width, _ = text_size(phase_text, 0.42 * A.s, 1)
            draw_text(
                frame,
                phase_text,
                (phase_x + phase_width - phase_text_width, row_y),
                0.42 * A.s,
                (180, 175, 170) if index <= active else (90, 85, 80),
                1,
            )
            cv2.line(
                frame,
                (phase_x, row_y + sc(A, 12)),
                (phase_x + phase_width, row_y + sc(A, 12)),
                (58, 54, 50),
                1,
                cv2.LINE_AA,
            )
...
def _legacy_compose_frame_c(
    ...
    """Range-camera layout copied faithfully from broadcast variant 2."""
    W, H = cfg.width, cfg.height
    A = assets
    frame = cover_resize(video_frame, W, H).astype(np.float32)
    frame *= A.scrim_mult
    ...
    if t < 0:
        ...
        status, s_color = "TEST COMPLETE", YELLOW
    sw = draw_spaced_text(
        frame, status, (-10000, -10000), 0.55 * A.s, s_color, 2, sc(A, 4)
    )
    sx = W - margin - sw
    sy = sc(A, 118)
    ...
    cv2.circle(
        frame,
        (sx - sc(A, 20), sy - sc(A, 7)),
        sc(A, 5),
        tuple(int(c * pulse) for c in s_color),
        -1,
        cv2.LINE_AA,
    )
    ...
    faint = roi.copy()
    area = np.vstack(
        [A.chart_pts, [[cwd - 1, chg - 1], [0, chg - 1]]]
    ).astype(np.int32)
    cv2.fillPoly(faint, [area], (255, 255, 255))
    frame[cyr:base_y + sc(A, 10), cxr:cxr + cwd] = cv2.addWeighted(
        faint, 0.12, roi, 0.88, 0
    )
    ...
    for label, pt in phases:
        ...
        cv2.fillPoly(
            frame,
            [pts_d],
            A.accent if reached else (80, 74, 68),
            cv2.LINE_AA,
        )
        t_color = WHITE if reached else (110, 104, 98)
        lw_ = draw_spaced_text(
            frame,
            label,
            (-10000, -10000),
            0.4 * A.s,
    ...
    cur_y = (
        int(np.interp(cur_x, A.chart_pts[:, 0], A.chart_pts[:, 1]))
        if A.chart_pts.shape[0]
        else chg
    )
    cv2.circle(
    ...
    peaks = [
        ("PEAK PRESSURE", f"{A.p_peak:.2f} {cfg.pressure_unit}"),
...
def compose_frame_stellar_console(
    video_frame: np.ndarray,
    cfg: Config,
    assets: OverlayAssets,
    t: float,
    ...
    """Template 3 copied from compose_frame_d in the supplied original file."""
    W, H = cfg.width, cfg.height
    A = assets
    frame = cover_resize(video_frame, W, H).astype(np.float32)
    frame *= A.scrim_mult
    ...
    if A.logo is not None:
        ...
        organization_name = config_text(cfg, "organization_name", "STELLAR KINETICS")
        draw_text(
            frame,
            organization_name,
            (x, sc(A, 86)),
        ...
        x += text_size(organization_name, 0.85 * A.s, 2)[0] + sc(A, 26)
    cv2.line(
        frame,
        (x, sc(A, 50)),
        (x, sc(A, 94)),
    ...
    if metadata:
        draw_text(
            frame, metadata, (x, sc(A, 128)), 0.4 * A.s,
            (150, 145, 140), 1, FONT_MONO,
        )
    ...
    site = config_text(cfg, "test_site", "ETLAQ SPACEPORT - DUQM, OMAN")
    stw, _ = text_size(site, 0.42 * A.s, 1, FONT_MONO)
    draw_text(
        frame,
        site,
        (W - margin - stw, sc(A, 102)),
        0.42 * A.s,
        GRAY,
        1,
        FONT_MONO,
    ...
    # Right: vertical pressure tape. There is deliberately no graph in D.
    ...
    frac = float(
        np.clip(pressure / (A.p_peak * 1.05) if A.p_peak else 0, 0, 1)
    )
    fh = int(tape_h * frac)
    if fh > 0:
        cv2.rectangle(
            frame,
            (tape_x, tape_y + tape_h - fh),
            (tape_x + tape_w, tape_y + tape_h),
            A.accent,
            -1,
        )
    ...
    cv2.line(
        frame,
        (tape_x - sc(A, 6), peak_y),
        (tape_x + tape_w + sc(A, 6), peak_y),
        WHITE,
        max(1, sc(A, 2)),
        cv2.LINE_AA,
    )
    pressure_limit = getattr(cfg, "pressure_limit", None)
    if pressure_limit is not None and float(pressure_limit) > 0 and A.p_peak > 0:
        limit_fraction = float(np.clip(float(pressure_limit) / (A.p_peak * 1.05), 0, 1))
        limit_y = tape_y + tape_h - int(tape_h * limit_fraction)
        cv2.line(
            frame,
            (tape_x - sc(A, 10), limit_y),
            (tape_x + tape_w + sc(A, 10), limit_y),
            YELLOW,
            max(1, sc(A, 2)),
            cv2.LINE_AA,
        )
        draw_text(
            frame,
            f"LIMIT {float(pressure_limit):g}",
            (tape_x - sc(A, 72), limit_y - sc(A, 8)),
            0.34 * A.s,
            YELLOW,
            1,
            FONT_MONO,
    ...
    for i in range(5):
        ...
        draw_text(
            frame,
            f"{val:.0f}",
            (tape_x + tape_w + sc(A, 12), vy + sc(A, 5)),
            0.38 * A.s,
            (125, 118, 112),
            1,
            FONT_MONO,
    ...
    draw_spaced_text(
        frame,
        lbl,
        (tape_x + tape_w // 2 - lw_ // 2, tape_y - sc(A, 16)),
        0.36 * A.s,
        (140, 133, 125),
        1,
        sc(A, 3),
    ...
    panel = roi.copy()
    panel[:] = (16, 11, 8)
    frame[con_y:con_y + con_h, con_x0:con_x1] = cv2.addWeighted(
        panel, 0.58, roi, 0.42, 0
    )
    cv2.rectangle(
        frame,
        (con_x0, con_y),
    ...
    pk_x = v_x + sc(A, 500)
    for i, (lbl2, val2) in enumerate(
        (
            ("PEAK P", f"{A.p_peak:.2f} {cfg.pressure_unit}"),
            (
                "PEAK T",
                f"{A.f_peak:.1f} {cfg.thrust_unit}" if A.has_thrust else "N/A",
            ),
    ...
    if cfg.show_phases:
        phases = [
            ...
            ("IGNITION", 0.0),
            (
                "FULL THRUST" if A.has_thrust else "PRESSURE RISE",
                min(0.6, A.t_end * 0.1),
            ),
            ("BURNOUT", A.t_end),
        ]
        active = max(
            (i for i, phase in enumerate(phases) if t >= phase[1]),
            default=0,
        )
        seq_x = div2 + sc(A, 42)
        row_h = sc(A, 32)
        ...
        for i, (label, pt) in enumerate(phases):
            ...
            dot_c = (
                A.accent
                if i < active
                else (WHITE if i == active else (58, 54, 50))
            )
            cv2.fillPoly(frame, [pts_d], dot_c, cv2.LINE_AA)
            t_color = (
                WHITE
                if i == active
                else ((150, 145, 140) if i < active else (85, 80, 75))
            )
            draw_spaced_text(
                frame,
                label,
    ...
    draw_text(
        frame,
        config_text(
            cfg, "footer_tagline", "STELLAR KINETICS - ENGINEERING THE VOID"
        ),
        (con_x0, fy),
        0.42 * A.s,
        (120, 114, 108),
    ...
    right_txt = " - ".join(
        filter(
            None,
            (
                config_text(cfg, "propellant", "LOX / PROPANE"),
                config_text(cfg, "camera_label", "CAM 01"),
                config_text(cfg, "capture_fps", "120 FPS"),
            ),
        )
    )
...
# Compatibility for drafts/code that imported the old third-theme function
# name. It now resolves to the supplied template D and can never add a graph.
compose_frame_range_line = compose_frame_stellar_console


THEME_COMPOSITORS = {
    "launch": compose_frame_launch,
    "mission_control": compose_frame_mission_control,
    "stellar_console": compose_frame_stellar_console,
}
...
def compose_frame(
    ...
    """Compose the selected broadcast theme through one stable entry point."""
    theme = normalize_broadcast_theme(getattr(cfg, "broadcast_theme", "launch"))
    compositor = THEME_COMPOSITORS.get(theme)
    if compositor is None:
        fail(f"Unknown broadcast theme: {theme}")
    return compositor(video_frame, cfg, assets, t, pressure, thrust)
...
def compose_overlay_bgra(
    ...
    """Return only the broadcast graphics as a straight-alpha BGRA frame.

    The established compositor contains a mixture of opaque drawing and
    translucent scrims. Sampling it over black and white mattes recovers the
    exact foreground and alpha without ever passing the source video through
    OpenCV. FFmpeg can then composite this layer over the original 10-bit HDR
    frames, preserving all untouched source pixels through lossless encoding.
    """
    shape = (cfg.height, cfg.width, 3)
    black = np.zeros(shape, dtype=np.uint8)
    white = np.full(shape, 255, dtype=np.uint8)
    on_black = compose_frame(black, cfg, assets, t, pressure, thrust)
    on_white = compose_frame(white, cfg, assets, t, pressure, thrust)

    transmission_rgb = (
        on_white.astype(np.float32) - on_black.astype(np.float32)
    ) / 255.0
    transmission = np.median(transmission_rgb, axis=2)
    alpha = np.clip(1.0 - transmission, 0.0, 1.0)
    alpha_u8 = np.clip(np.rint(alpha * 255.0), 0, 255).astype(np.uint8)

    straight_bgr = np.zeros_like(on_black, dtype=np.float32)
    visible = alpha_u8 > 0
    straight_bgr[visible] = (
        on_black[visible].astype(np.float32) / alpha[visible, None]
    )
    straight_bgr = np.clip(
        np.rint(straight_bgr), 0, 255
    ).astype(np.uint8)
    return np.dstack((straight_bgr, alpha_u8))
    ...
    with tempfile.TemporaryDirectory(prefix="rocket_overlay_") as tmpdir:
        ...
        while True:
            ...
            if telemetry_t < 0 or telemetry_t > assets.t_end + 0.5:
                p, f = 0.0, 0.0
            writer.write(compose_frame(frame, cfg, assets, telemetry_t, p, f))
            idx += 1
            if frame_count > 0:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--broadcast-theme",
        choices=tuple(BROADCAST_THEMES),
        default="launch",
        help="Broadcast layout used for the rendered graphics.",
    )
...
def main() -> None:
    try:
        ...
        render(cfg)
    except BroadcastOverlayError as exc:
        print(f"\nERROR: {exc}\n", file=sys.stderr)
